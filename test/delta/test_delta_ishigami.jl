...
samples = load(joinpath(@__DIR__, "../data/delta/py_ishigami/py_samples.csv"), header_exists=false, colnames = ["x1", "x2", "x3"]) |> DataFrame
samples = Matrix(samples)

# check ishigami
python_Y = load(joinpath(@__DIR__, "../data/delta/py_ishigami/py_ishigami.csv"), header_exists=false) |> DataFrame
...
# python results
py_firstorder = load(joinpath(@__DIR__, "../data/delta/py_ishigami/py_firstorder.csv"), header_exists=false) |> DataFrame
py_delta = load(joinpath(@__DIR__, "../data/delta/py_ishigami/py_delta.csv"), header_exists=false) |> DataFrame
py_firstorder_conf = load(joinpath(@__DIR__, "../data/delta/py_ishigami/py_firstorder_conf.csv"), header_exists=false) |> DataFrame
py_delta_conf = load(joinpath(@__DIR__, "../data/delta/py_ishigami/py_delta_conf.csv"), header_exists=false) |> DataFrame
